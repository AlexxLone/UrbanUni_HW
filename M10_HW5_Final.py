...
# В линейном режиме чтение данных заняло: 0:00:03.728650
# В многопроцессорном режиме чтение данных заняло: 0:00:08.476417
...
files_path = 'C:/Temp/'  # директория с рабочими файлами, поменяйте на вашу
...
def read_info(filename):
    # all_data_local - возвращаемый результат. В многопроцессном режиме содержит лишь список
    # для вызвавшего процесса (т.е. для обрабатываемого процессом файла).
    # Далее объединяется в multiprocessing.map
    all_data_local = []
    with open(filename, 'r', encoding='UTF-8') as file_:
        for line in file_:
            all_data_local.append(line)
    return all_data_local

# Важное замечание!:
# Поскольку all_data_multi = (pool.map(read_info, file_list)) вернет нам список списков -
# в линейном режиме мы также сформируем список списков, где каждый внутренний список будет содержать
# содержимое отдельного файла. Так даже более наглядно распределение в мультипроцессинге.
...
if __name__ == '__main__':
    ...
    if len(all_data_mono) != len(all_data_multi):
        ...
        differ = True
    elif all_data_mono != all_data_multi:
        print('Полученные списки различны!')
        differ = True
    if differ is False:
        print('Отличий нет')
